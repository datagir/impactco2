...
test('Livraison simulator', async ({ page }) => {
  ...
  await expect(page.getByTestId('category-magasindouce-value')).toHaveText('366')
  ...
  await expect(page.getByTestId('livraison-pointrelais')).toHaveText(
    "Transport utilisé pour aller chercher l'objetVoiture thermiqueVoiture électrique7 km Distance parcourue pour Livraison en point relais à 7km"
  )
  await page.getByRole('button', { name: 'Augmenter le nombre de km' }).first().click()
  await expect(page.getByTestId('livraison-pointrelais')).toHaveText(
    "Transport utilisé pour aller chercher l'objetVoiture thermiqueVoiture électrique8 km Distance parcourue pour Livraison en point relais à 7km"
  )
